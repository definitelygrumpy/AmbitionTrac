...
interface PassionInputProps {
    onSetPassion: (passion: string) => void;
}

const PassionInput: React.FC<PassionInputProps> = ({ onSetPassion }) => {
    const [passion, setPassion] = useState('');

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (passion.trim()) {
            onSetPassion(passion.trim());
        }
    };

    return (
        <div className="text-center p-8 bg-white dark:bg-dark-card rounded-xl shadow-lg animate-fade-in">
            <h2 className="text-3xl font-bold text-gray-800 dark:text-white">What is your passion?</h2>
            <p className="mt-2 text-gray-600 dark:text-gray-400">Let's turn your passion into achievable steps.</p>
